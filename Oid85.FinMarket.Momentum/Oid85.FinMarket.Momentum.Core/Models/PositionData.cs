...
namespace Oid85.FinMarket.Momentum.Core.Models
{
    ...
    public class PositionData
    {
        ...
        public double CurrentStopSizePercent => Candle.Close == 0.0 || StopPrice == 0.0 ? 0.0 : (CurrentStopSize / Candle.Close * 100.0).RoundTo(2);
        public bool IsBreakEvenStop { get; set; } = false;
        public double EntryPrice { get; set; } = 0.0;
        public double ProfitPercent => Candle.Close == 0.0 || EntryPrice == 0.0 ? 0.0 : ((Candle.Close - EntryPrice) / Candle.Close * 100.0).RoundTo(2);
