...
namespace Oid85.FinMarket.Momentum.Application.Strategies
{
    public class MomentumStrategyVersion1 : MomentumStrategy
    {
        ...
        public override List<string> GetDescription()
        {
            return [
                "Версия 1",
                $"Дни ребалансировки: {string.Join(", ", ParameterRebalanceDays).Trim()} каждого месяца",
                $"Период отбора тикеров лидеров роста для моментума {ParameterPeriod} дней",
                $"Стоп-лосс расчитывается как двойной средний размер тела свечи за {ParameterPeriod} дней",
